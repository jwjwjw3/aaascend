...
def load_resnet_model(model='50'):
    if model == '50':
...
def prepare_model(num_epochs, model_depth, save_path, data_dir):
    ...
    model = load_resnet_model(model_depth)
    model.to("cpu")
    save_name = 'resnet' + model_depth + "cifar10.pth"
    model.to(device)
...
def main():
    ...
    save_name = 'resnet' + args.model_depth + "_cifar10_trained"
    model_path = str(models_dir / save_name) + '.pt'
    onnx_path = str(models_dir / save_name) + '.onnx'
    ...
    if args.train:
        prepare_model(args.num_epochs, args.model_depth, model_path, data_dir)
